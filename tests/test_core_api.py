...
def test_load_from_samples_rejects_legacy_only_pkl(tmp_path):
    legacy_path = tmp_path / "legacy_only_samples.pkl"
    legacy_path.write_bytes(b"legacy-pickle-placeholder")

    with pytest.raises(FileNotFoundError, match="Posterior bundle not found"):
        jaxqsofit.load_from_samples(
            filename="legacy_only",
            output_path=str(tmp_path),
            plot_fig=False,
...
def test_load_from_samples_autodiscovery_ignores_legacy_pkl(tmp_path):
    legacy_path = tmp_path / "legacy_auto_samples.pkl"
    legacy_path.write_bytes(b"legacy-pickle-placeholder")

    with pytest.raises(FileNotFoundError, match="compressed posterior bundle"):
        jaxqsofit.load_from_samples(
            output_path=str(tmp_path),
            plot_fig=False,
            plot_diagnostics=False,
